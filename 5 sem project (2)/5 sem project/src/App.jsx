...
// Utility component to display key metrics (Styled Card)
const DashboardCard = ({ icon: Icon, title, value, unit, status, colorClass }) => {
  // colorClass is the border color class (e.g., 'border-b-green-500')
  // We extract the base color (e.g., 'green') for the background/status chip.
  const colorParts = colorClass.split('-');
  const baseColor = colorParts[2]; // e.g., 'green'
    ...
    <div className={`p-6 rounded-xl shadow-lg transition-all duration-300 hover:shadow-xl bg-white border-b-4 ${colorClass}`}>
      <div className="flex justify-between items-start">
        <div className="flex items-center space-x-3">
          {/* Icon background using the base color at 10% opacity */}
          <div className={`p-3 rounded-full text-${baseColor}-700 bg-${baseColor}-100`}>
            <Icon className="w-6 h-6" />
          </div>
          <h3 className="text-lg font-semibold text-gray-700">{title}</h3>
        </div>
        {/* Status chip using the base color */}
        <span className={`text-xs font-semibold px-3 py-1 rounded-full text-white ${bgColorClass}`}>
          {status}
        </span>
      </div>
      <div className="mt-4">
  ...
  useEffect(() => {
    const loadAllData = async () => {
      ...
      if (!aqi || !carbon || !forecast) {
          ...
          // Set a user-friendly error if the backend is confirmed down, otherwise allow partial load
          if (!aqi && !carbon && !forecast) {
              setError(`Could not retrieve environmental data. Please ensure the backend server is running on port 3000.`);
          }
      } else {
          setError(null);
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      <main className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div className="mb-8 p-6 bg-white rounded-xl shadow-lg border-l-4 border-emerald-500">
        ...
        {/* Section 1: Core Monitoring Metrics (4 Card Grid) */}
        ...
        {/* Section 2: Remote Sensing & Prediction Detail (Two-Column Layout) */}
        <section className="grid lg:grid-cols-3 gap-6 mb-12">
            
            {/* Pollution Forecast Detail (Wider Column) */}
            ...
            {/* NDVI Remote Sensing (Taller, narrower column) */}
            <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
                ...
                <img 
                    ...
                    onError={(e) => { e.target.onerror = null; e.target.src="https://placehold.co/600x400/228B22/FFFFFF?text=NDVI+Image+Unavailable" }}
                />
        ...
        {/* Section 3: Personalized Recommendations (Grid) */}
